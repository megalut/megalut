...
def fitsdenoise(infile, outfile, thresholds, crop=None):
	"""
	The high-level wrapper
	
	crop = 1024 would take only the first 1024 x 1024 pixels (for testing...)
	
	"""
	starttime = datetime.now()	
	
	print "Denoising %s..." % (infile)
	print "(Output will be written into %s)" % (outfile)
	img = fromfits(infile)
	if crop:
		img = img[:crop, :crop]
			
	den = cyclespinmap(img, htdenoise, n=len(thresholds), thresholds=thresholds)

	tofits(den, outfile)
	print "Done"
	endtime = datetime.now()
	print "This denoising took %s" % (str(endtime - starttime))
...
def smooth(a, s):
	...
	return sn.filters.gaussian_filter(a, s, mode='nearest')


def addgauss(a, c1, c2, fwhm, i0) :
        sig = (fwhm / (2. * np.sqrt(2.*np.log(2.))))**2.
        g = lambda x,y: i0*np.exp((-(x-c1+0.5)**2. - (y-c2+0.5)**2)/(2.*sig))
        a += np.fromfunction(g, a.shape) 
...
def downsample(a):
	"""
	2 by 2 downsampling (takes the mean value of 4 pixels)
	"""
	shape = a.shape
	lenShape = len(shape)
	newshape = np.asarray(shape)/2
...
def upsample(a):
	"""
	2 by 2 upsampling (divides the pixel values by 4)
	"""
	shape = a.shape
	newshape = np.asarray(shape)*2
