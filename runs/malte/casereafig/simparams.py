...
class Simple1(megalut.sim.params.Params):
	...
	def draw(self, ix, iy, nx, ny):
		...
		tru_flux = 20.0 + 100.0 * (1.0 - (float(iy)/(float(ny)-1.0)))
		tru_rad = np.random.uniform(3.0, 5.0)
		tru_sersicn = 2.0
		
		tru_g = contracted_rayleigh(0.25, 0.6, 4)
...
class Simple2(megalut.sim.params.Params):
	"""
	Sersics with shear.
	Single PSF.
	"""
	...
	def draw(self, ix, iy, nx, ny):
		...
		tru_flux = np.random.uniform(20.0, 120.0)
		tru_rad = np.random.uniform(3.0, 10.0)
		tru_sersicn = np.random.uniform(1.0, 4.0)
