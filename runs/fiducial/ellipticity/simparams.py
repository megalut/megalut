...
class EllipticityVarPSF(megalut.sim.params.Params):
	...
	def load_psf_field(self, indir, nfield):
		self.psf_g1_interp, self.psf_g2_interp = tl.io.readpickle(os.path.join(indir, "field-{:03}.pkl".format(nfield)))
	...
	def draw(self, ix, iy, nx, ny):
		...
		########## PSF ##########
		pos_psf_x, pos_psf_y = np.random.uniform(size=2)
		tru_psf_sigma = 0.58174 #0.58174 # = 0.137(FWHM) / 2.355 * 10. (px scale is 1" in megalut, Euclid is 0.1") 
		tru_psf_g1 = self.psf_g1_interp(pos_psf_x, pos_psf_y)
		tru_psf_g2 = self.psf_g2_interp(pos_psf_x, pos_psf_y)
